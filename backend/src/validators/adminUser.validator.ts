...
const organizationFields = {
  organizationCode: z.string().trim().min(2).max(40)
  ...
  countryCode: z.string().trim().length(2).default("XK")
    .transform((value) => value.toUpperCase())
};
...
export const createPatientSchema = z.object({
  ...
  maritalStatus: z.enum(["SINGLE", "MARRIED", "DIVORCED", "WIDOWED", "OTHER", "UNKNOWN"]).default("UNKNOWN"),
  ...
  city: z.string().trim().max(100).optional().default(""),
  postalCode: z.string().trim().max(20).optional().default(""),
  countryCode: z.string().trim().length(2).default("XK").transform((value) => value.toUpperCase())
});
