...
export function PatientProfilePage() {
  ...
  if (loading) {
    return (
      <div className="patient-profile-state">
  ...
  if (!profile || !form) {
    return (
      <div className="patient-profile-state patient-profile-state--error">
        <AlertCircle />
        <span>{error ?? "Profile information is unavailable."}</span>
    ...
    <div className="patient-profile-page">
      <section className="patient-profile-hero">
        ...
        <div>
          ...
          <p>Review your identity details and keep your contact information current.</p>
        </div>
        <div className="patient-profile-number">
          <small>Patient number</small>
          <strong>{profile.patientNumber}</strong>
        </div>
      </section>

      {notice && (
        <div className="patient-profile-notice patient-profile-notice--success">
          <Check size={17} />
          <span>{notice}</span>
      ...
      {error && (
        <div className="patient-profile-notice patient-profile-notice--error">
          <AlertCircle size={17} />
          <span>{error}</span>
      ...
      <form className="patient-profile-grid" onSubmit={save}>
        <section className="patient-profile-panel">
          ...
          <p className="patient-profile-help">
            These fields are protected. Contact an administrator if a correction is required.
          </p>
        ...
        <section className="patient-profile-panel">
          <header>
            <div>
              ...
              <h3>Contact and profile details</h3>
            </div>
            {!editing && (
              <button
          ...
          <div className="patient-profile-fields">
            ...
            <Input
              ...
              disabled={!editing}
              change={(occupation) => setForm({ ...form, occupation })}
            />
            <Select
              label="Marital status"
              value={form.maritalStatus}
              disabled={!editing}
              options={["UNKNOWN", "SINGLE", "MARRIED", "DIVORCED", "WIDOWED", "OTHER"]}
              change={(maritalStatus) =>
                setForm({
                  ...form,
                  maritalStatus: maritalStatus as UpdatePatientProfileInput["maritalStatus"]
                })
              }
            />
            <Select
              ...
              disabled={!editing}
              options={["UNKNOWN", "NEVER", "FORMER", "CURRENT"]}
              change={(smokingStatus) =>
                setForm({
                  ...form,
                  smokingStatus: smokingStatus as UpdatePatientProfileInput["smokingStatus"]
                })
              }
            />
            <Input
              ...
              disabled={!editing}
              change={(addressLine1) => setForm({ ...form, addressLine1 })}
            />
            <Input
              label="Address line 2"
              value={form.addressLine2}
              disabled={!editing}
              change={(addressLine2) => setForm({ ...form, addressLine2 })}
            />
            <Input
              label="City"
              value={form.city}
            ...
            <Input
              ...
              disabled={!editing}
              change={(postalCode) => setForm({ ...form, postalCode })}
            />
            <Input
              label="Country code"
              ...
              required
              disabled={!editing}
              change={(countryCode) => setForm({ ...form, countryCode: countryCode.toUpperCase() })}
            />
          </div>
          {editing && (
            <footer>
...
function Select({
  ...
  return (
    <label>
      {label}
      <select value={value} disabled={disabled} onChange={(event) => change(event.target.value)}>
        {options.map((option) => (
